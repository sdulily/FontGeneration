...
from models.Muti_attention import Attention
...
class Generator(nn.Module):   
    ...
    def forward(self, x_src, s_ref, component):
        c_src = self.cnt_encoder(x_src, component)
...
class ContentEncoder(nn.Module):
    def __init__(self, nf_cnt, n_downs, n_res, norm, act, pad, use_sn=False):
        ...
        self.attn = Attention(img_size=16, dim=512, num_heads=8)

    def forward(self, cnt, component):  # cnt: [3, 128, 128]  component:[comp1, comp2, ..., comp15]
        cnt_feat = self.model(cnt)
        comp_feat = []
        for i in range(len(component)):
            comp = self.model(component[i])
            comp_feat.append(comp)

        result = self.attn(cnt_feat, comp_feat)
        return result
...
if __name__ == '__main__':
    ...
    C = GuidingNet(128)
    G = Generator(128, 128, 4)
    x_in = torch.randn(4, 3, 128, 128)
    compo = torch.randn(4, 3, 128, 128)
    com_list = []
    for i in range(15):
        com_list.append(compo)

    cont = G.cnt_encoder(x_in, com_list)
    print(cont.shape)
    sty = C.moco(x_in)
    print(sty.shape)
