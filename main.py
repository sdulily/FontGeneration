...
parser.add_argument('--dataset', default='font', help='Dataset name to use',
                    choices=['afhq', 'afhq_dog', 'afhq_wild', 'animal_faces', 'photo2ukiyoe', 'summer2winter', 'lsun_car', 'ffhq'])
...
parser.add_argument('--output_k', default=6, type=int, help='Total number of classes to use')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Run
    validationFunc(val_loader, networks, 0, args, {'logger': logger})
...
def get_loader(args, dataset):
    ...
    val_loader = {'VAL': val_loader, 'VALSET': val_dataset if not args.dataset in ['afhq_cat', 'afhq_dog', 'afhq_wild'] else dataset['val'], 'TRAINSET': train_dataset}
    if 'afhq' in args.dataset:
