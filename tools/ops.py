...
def calc_contrastive_loss(args, query, key, queue, temp=0.07):
    ...
    logit_neg = torch.bmm(query.view(N, 1, -1), queue.view(N, -1, K))
    #logit_neg = torch.mm(query.view(N, -1), queue.t().view(-1, K))
    logit_neg = torch.zeros((0, K), dtype=torch.float).cuda(args.gpu)
    for i in range(N):
        a = torch.zeros((1, 0), dtype=torch.float).cuda(args.gpu)
        for index in range(K):
            neg = torch.mm(query[i].view(1, -1), queue[i][index].view(-1, 1))
            a = torch.cat((a, neg), dim=1)

        logit_neg = torch.cat((logit_neg, a), dim=0)

    logit = torch.cat([logit_pos.view(N, 1), logit_neg.view(N, K)], dim=1)
...
def initialize_queue(model_k, device, train_loader, feat_size=128):
    ...
    for _, (source, reference, b, c) in enumerate(train_loader):
        x_k = reference[1]
        x_k = x_k.cuda(device)
        outs = model_k(x_k)
        k = outs['cont']
