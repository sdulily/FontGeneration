    ...
    with torch.no_grad():
        ...
        for cls_idx in range(len(args.att_to_use)):
            ...
            for sample_idx in range(len(tmp_iter)):
                source, reference, _, _, comp_list, _ = next(tmp_iter)
                x_ = source
                ...
                if tmp_ref_sample is None:
                    tmp_ref_sample = reference.clone()
                else:
                    tmp_ref_sample = torch.cat((tmp_ref_sample, reference), 0)

                if len(tmp_comp_sample) == 0:
                    tmp_comp_sample = comp_list
                else:
                    for i in range(len(tmp_comp_sample)):
                        tmp_comp_sample[i] = torch.cat((tmp_comp_sample[i], comp_list[i]), 0)
            # x_each_cls.append(tmp_sample)
            x_each_cls = tmp_sample
            ref_each_cls.append(tmp_ref_sample)
            # comp_each_cls.append(tmp_comp_sample)
            comp_each_cls = tmp_comp_sample
    #######
    if epoch >= args.fid_start:
        # val_iter = iter(val_loader)
        # cluster_grid = [[] for _ in range(args.output_k)]
        # for _ in tqdm(range(len(val_loader))):
        #     x, ref, y = next(val_iter)
        #     x = x.cuda(args.gpu)
        #     outs = C(x)
        #     feat = outs['cont']
        #     logit = outs['disc']
        ...
        with torch.no_grad():
            for src_idx in [0]:
                rnd_src_idx = torch.randperm(x_each_cls.size(0))[:args.val_batch]
                # x_src = x_each_cls[src_idx][:args.val_batch, :, :, :].cuda(args.gpu, non_blocking=True)
                x_src_rnd = x_each_cls[rnd_src_idx].cuda(args.gpu, non_blocking=True)
                comp_rnd_list = []
                for i in range(15):
                    comp_rnd = comp_each_cls[i][rnd_src_idx].cuda(args.gpu, non_blocking=True)
                    comp_rnd_list.append(comp_rnd)
                rnd_ref_idx = torch.randperm(ref_each_cls[src_idx].size(0))[:args.val_batch]
                x_ref_rnd = ref_each_cls[src_idx][rnd_ref_idx].cuda(args.gpu, non_blocking=True)
                # x_ref = np.random.choice(ref_each_cls[src_idx], size=args.val_batch).cuda(args.gpu, non_blocking=True)
                c_src = G_EMA.cnt_encoder(x_src_rnd, comp_rnd_list)
                s_ref = C_EMA(x_ref_rnd, sty=True)
                output = G_EMA.decode(c_src, s_ref)
                result = torch.cat((x_src_rnd, x_ref_rnd, output), dim=0)
                vutils.save_image(result,
                                  os.path.join(args.res_dir, '{}_RNDEMA_{}_{}.jpg'.format(args.gpu, epoch + 1, src_idx)),
                                  normalize=True, nrow=args.val_batch)
        ...
        #         for ref_idx in [20, 50, 100, 200, 301, 350]:
